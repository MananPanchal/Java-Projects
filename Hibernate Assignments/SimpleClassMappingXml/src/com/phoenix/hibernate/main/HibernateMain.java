...
public class HibernateMain {
	...
	public static void main(String[] args) {
		...
		/*
		 * Product product = ss.get(Product.class, 3001); System.out.println(product);
		 */
		// Insert an Object as a new row
		// Product p = new Product(1001, "Mobile", "Mi", 15000.00f);
		// ss.save(p);
	...
	/*	// Product p = ss.load(Product.class, 21); No row
		...
		// Update row price using an object
		 ...
		 Product p = ss.get(Product.class, 1001); if(p != null) {
		 p.setPrice(p.getPrice() + 3000.00f); p.setBrand("Redmi"); ss.update(p); }
		 else System.out.println("Sorry! Product is not found!!");
		 
		 p = ss.get(Product.class, 1001); if(p != null) System.out.println(p); else
